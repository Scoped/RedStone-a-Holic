...
public class Block_Torch_BrownStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map brownstoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_BrownStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle2");
        this.setIconTorchLit("Torch2");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_BROWNSTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!brownstoneUpdateInfoCache.containsKey(par1World))
        {
        	brownstoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)brownstoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 0.7D, 0.5D, 0.4D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 0.7D, 0.5D, 0.4D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 0.7D, 0.5D, 0.4D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 0.7D, 0.5D, 0.4D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 0.7D, 0.5D, 0.4D); //Red, Green, Blue
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.BrownStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.BrownStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.BrownStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.BrownStoneTorchIdle.blockID || par1 == ModBlocks.BrownStoneTorchActive.blockID;
    }
}