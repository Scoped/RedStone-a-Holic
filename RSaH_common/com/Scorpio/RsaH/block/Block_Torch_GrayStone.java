...
public class Block_Torch_GrayStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map graystoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_GrayStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle5");
        this.setIconTorchLit("Torch5");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_GRAYSTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!graystoneUpdateInfoCache.containsKey(par1World))
        {
        	graystoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)graystoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 0.5D, 0.5D, 0.5D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 0.5D, 0.5D, 0.5D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 0.5D, 0.5D, 0.5D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 0.5D, 0.5D, 0.5D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 0.5D, 0.5D, 0.5D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.GrayStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.GrayStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.GrayStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.GrayStoneTorchIdle.blockID || par1 == ModBlocks.GrayStoneTorchActive.blockID;
    }
}