...
import com.Scorpio.RsaH.lib.RedstoneAHolicUpdateInfo;
import com.Scorpio.RsaH.lib.Strings;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class Block_Torch_LightBlueStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map lightbluestoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_LightBlueStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle7");
        this.setIconTorchLit("Torch7");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_LIGHTBLUESTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!lightbluestoneUpdateInfoCache.containsKey(par1World))
        {
        	lightbluestoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)lightbluestoneUpdateInfoCache.get(par1World);

        if (par5)
        {
            list.add(new RedstoneAHolicUpdateInfo(par2, par3, par4, par1World.getTotalWorldTime()));
        }
        ...
        {
            RedstoneAHolicUpdateInfo redstoneupdateinfo = (RedstoneAHolicUpdateInfo)list.get(i1);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 0.4D, 0.6D, 0.7D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 0.4D, 0.6D, 0.7D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 0.4D, 0.6D, 0.7D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 0.4D, 0.6D, 0.7D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 0.4D, 0.6D, 0.7D); //-1.0D, 0.8D, 3.0D);
            }
        }
    }
    ...
    while (list != null && !list.isEmpty() && par1World.getTotalWorldTime() - ((RedstoneAHolicUpdateInfo)list.get(0)).updateTime > 60L)
    {
        list.remove(0);
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.LightBlueStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.LightBlueStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.LightBlueStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.LightBlueStoneTorchIdle.blockID || par1 == ModBlocks.LightBlueStoneTorchActive.blockID;
    }
}