...
public class Block_Torch_LimeGreenStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map limegreenstoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_LimeGreenStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle9");
        this.setIconTorchLit("Torch9");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_LIMEGREENSTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!limegreenstoneUpdateInfoCache.containsKey(par1World))
        {
        	limegreenstoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)limegreenstoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, -1.0D, 0.9D, 0.3D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, -1.0D, 0.9D, 0.3D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, -1.0D, 0.9D, 0.3D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, -1.0D, 0.9D, 0.3D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, -1.0D, 0.9D, 0.3D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.LimeGreenStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.LimeGreenStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.LimeGreenStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.LimeGreenStoneTorchIdle.blockID || par1 == ModBlocks.LimeGreenStoneTorchActive.blockID;
    }
}