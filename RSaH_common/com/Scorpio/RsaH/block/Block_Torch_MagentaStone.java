...
public class Block_Torch_MagentaStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map magentastoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_MagentaStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("Torchidle10");
        this.setIconTorchLit("Torch10");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_MAGENTASTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!magentastoneUpdateInfoCache.containsKey(par1World))
        {
        	magentastoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)magentastoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 0.6D, 0.0D, 0.5D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 0.6D, 0.0D, 0.5D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 0.6D, 0.0D, 0.5D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 0.6D, 0.0D, 0.5D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 0.6D, 0.0D, 0.5D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.MagentaStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.MagentaStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.MagentaStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.MagentaStoneTorchIdle.blockID || par1 == ModBlocks.MagentaStoneTorchActive.blockID;
    }
}