...
public class Block_Torch_PinkStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map pinkstoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_PinkStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle13");
        this.setIconTorchLit("Torch13");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_PINKSTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!pinkstoneUpdateInfoCache.containsKey(par1World))
        {
        	pinkstoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)pinkstoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 1.0D, 0.4D, 0.6D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 1.0D, 0.4D, 0.6D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 1.0D, 0.4D, 0.6D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 1.0D, 0.4D, 0.6D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 1.0D, 0.4D, 0.6D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.PinkStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.PinkStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.PinkStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.PinkStoneTorchIdle.blockID || par1 == ModBlocks.PinkStoneTorchActive.blockID;
    }
}