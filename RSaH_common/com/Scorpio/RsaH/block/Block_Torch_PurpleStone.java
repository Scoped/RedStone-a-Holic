...
public class Block_Torch_PurpleStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneAHolicUpdateInfo. Key of map is World. */
    private static Map purplestoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_PurpleStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle14");
        this.setIconTorchLit("Torch14");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_PURPLESTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!purplestoneUpdateInfoCache.containsKey(par1World))
        {
        	purplestoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)purplestoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 0.8D, -1.0D, 0.8D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 0.8D, -1.0D, 0.8D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 0.8D, -1.0D, 0.8D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 0.8D, -1.0D, 0.8D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 0.8D, -1.0D, 0.8D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.PurpleStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.PurpleStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.PurpleStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.PurpleStoneTorchIdle.blockID || par1 == ModBlocks.PurpleStoneTorchActive.blockID;
    }
}