...
public class Block_Torch_WhiteStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneUpdateInfo. Key of map is World. */
    private static Map whitestoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_WhiteStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle11");
        this.setIconTorchLit("Torch11");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_WHITESTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!whitestoneUpdateInfoCache.containsKey(par1World))
        {
        	whitestoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)whitestoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 10.0D, 10.0D, 10.0D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 10.0D, 10.0D, 10.0D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 10.0D, 10.0D, 10.0D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 10.0D, 10.0D, 10.0D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 10.0D, 10.0D, 10.0D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.WhiteStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.WhiteStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.WhiteStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.WhiteStoneTorchIdle.blockID || par1 == ModBlocks.WhiteStoneTorchActive.blockID;
    }
}