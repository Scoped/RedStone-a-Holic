...
public class Block_Torch_YellowStone extends BlockTorchRSaH
{
    /** Map of ArrayLists of RedstoneUpdateInfo. Key of map is World. */
    private static Map yellowstoneUpdateInfoCache = new HashMap();
    
	protected Block_Torch_YellowStone(int id, boolean isLite)
	{
		super(id, isLite);
        this.isTorchActive = isLite;
        this.setIconTorch("TorchIdle4");
        this.setIconTorchLit("Torch4");
        this.setTickRandomly(true);
        this.setCreativeTab((CreativeTabs)null);
		this.setUnlocalizedName(Strings.BLOCK_TORCH_YELLOWSTONE_NAME);
	}

	private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        if (!yellowstoneUpdateInfoCache.containsKey(par1World))
        {
        	yellowstoneUpdateInfoCache.put(par1World, new ArrayList());
        }

        List list = (List)yellowstoneUpdateInfoCache.get(par1World);
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        if (this.isTorchActive)
        {
            ...
            if (l == 1)
            {
                par1World.spawnParticle("reddust", d0 - d4, d1 + d3, d2, 1.5D, 1.5D, -1.0D);
            }
            else if (l == 2)
            {
                par1World.spawnParticle("reddust", d0 + d4, d1 + d3, d2, 1.5D, 1.5D, -1.0D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 - d4, 1.5D, 1.5D, -1.0D);
            }
            else if (l == 4)
            {
                par1World.spawnParticle("reddust", d0, d1 + d3, d2 + d4, 1.5D, 1.5D, -1.0D);
            ...
            {
                par1World.spawnParticle("reddust", d0, d1, d2, 1.5D, 1.5D, -1.0D);
            }
        }
    }
    ...
    if (this.isTorchActive)
    {
        if (flag)
        {
            par1World.setBlock(par2, par3, par4, ModBlocks.YellowStoneTorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    ...
    {
        par1World.setBlock(par2, par3, par4, ModBlocks.YellowStoneTorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
    }
    }
    ...
    {
        return ModBlocks.YellowStoneTorchActive.blockID;
    }
    ...
    {
        return par1 == ModBlocks.YellowStoneTorchIdle.blockID || par1 == ModBlocks.YellowStoneTorchActive.blockID;
    }
}